...
for entry in json_celebrations:
    if "names" in entry["celebrations"]:
        for name in entry["celebrations"]["names"]:
            assert re.search(r'\s', name) is None, "{} failed the whitespace check".format(name)
            assert re.search(r'[^Α-Ω]', name) is None, "{} failed the greek letters check".format(name)
